...
| **`password authentication failed for user "..."`** (migrate/app loglarında) | Eski `pgdata` volume'ü, `.env`'deki **yeni DB parolasıyla uyuşmuyor**. Postgres kullanıcı/parolayı **yalnız ilk açılışta** (boş veri dizini) gömer; yeniden-kurulumda klasörü silseniz bile **volume kalır** ve eski parolayı saklar. | Eski veritabanını sıfırlayın: **`docker compose down -v`** → ardından `docker compose up -d --build` (ya da `setup.ps1`). ⚠️ `-v` mevcut tarama verisini **siler**; yalnız kasıtlı creds değişikliğinde/temiz kurulumda kullanın. Etkin creds'i `docker compose config` çıktısında `POSTGRES_*` ile doğrulayın. Veriyi korumak istiyorsanız bunun yerine `.env`'deki parolayı **eski (ilk kurulumdaki) değerle** eşitleyin. |
