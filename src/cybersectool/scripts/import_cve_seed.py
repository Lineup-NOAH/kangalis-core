...
async def import_seed(seeddir: Path, *, force: bool = False) -> str:
    missing = [t for t in _TABLES if not (seeddir / f"{t}.csv.gz").exists()]
    if missing:
        ...
        async with conn.transaction():
            for table in _TABLES:
                ...
                with gzip.open(seeddir / f"{table}.csv.gz", "rb") as fh:
                    await conn.copy_to_table(table, source=fh, format="csv", header=True)
            # cpe_matches.id serial dizisini en yüksek id'ye ilerlet (sonraki INSERT çakışmasın).
            await conn.execute(
